...
سیستم نصب نیستند. `dev python` می‌زنی و همان لحظه ساخته می‌شود (`dev agent`, `python`, `rust`, `go`, `web`, `build`, `cli`, `media`, `nix`, `audit`, `box`).

**sandbox برای چیزی که نخوانده‌ای.** `box` یک فضای جدا با معماری **Home-Swap** می‌سازد که `~/.ssh` و
کلیدهایت در آن **وجود ندارند** — نه اینکه اجازه‌ی خواندن نداشته باشند. مسیر کاری منحصراً `/work` است و Shebangهای پایتون ۱۰۰٪ کار می‌کنند.
...
**۲. هر محیطی که یک بار وارد شوی، از زباله‌روب جان سالم می‌برد.** ورود، یک GC
